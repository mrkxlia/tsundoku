...
- python
- データ分析
summary: 'データワークスペースサービス『Morph』のオープンソース版を手元の環境に導入し試した手順を紹介しています。

  Mac環境でのpyenvによるPythonの導入や、morph-dataのインストール、Node.jsのセットアップを経てサーバーを起動するまでの流れを解説。

  サンプルアプリを短時間で簡単にローカル環境で起動でき、データ分析やアプリケーション構築の効率化に期待が持てるとまとめています。'
